...
function draw(transform) {
  for (let row = 0; row < tiles.length; row++) {
    for (let column = 0;column < tiles[row].length; column++) {
      ...
      const p1 = rotate(tile.point.copy()).add(100, 100).rotateX(Math.PI/3);
      const p2 = rotate(tile.point.add(tileSize, 0)).add(100, 100).rotateX(Math.PI/3);
      const p3 = rotate(tile.point.add(tileSize, tileSize)).add(100, 100).rotateX(Math.PI/3);
      const p4 = rotate(tile.point.add(0, tileSize)).add(100, 100).rotateX(Math.PI/3);
...
canvas.addEventListener('click', (event) => {
  ...
  const transformedPoint = new Point(layerX, layerY).rotateX(-Math.PI/3).add(-100, -100)
